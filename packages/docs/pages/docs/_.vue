<template>
  ...
  >
    ...
    >
      <article class="prose prose-dark max-w-none lg:px-8">
        <h1 v-if="!document.hideFullTitle" class="flex items-center justify-between mb-5 text-2xl font-semibold">
          {{ document.title }}
          <Badge v-if="document.badge">{{ document.badge }}</Badge>
        </h1>
        <div v-if="document.subtitle" class="-mt-4">
          <p class="text-gray-400">{{ document.subtitle }}</p>
        </div>
...
export default {
  name: 'PageSlug',
  layout ({ store }) {
    return store.state.settings.layout || 'docs'
  },
