...
function themeModule () {
  ...
  hook('content:file:beforeInsert', (document) => {
    ...
    if (_dir === '/crates' || _dir === '/links' || _dir === '/api') {
      document.path = path.join('/', options.i18n.defaultLocale, _dir, _slug)
    }

    document.to = path.join('/', 'docs', _dir, _slug)
    document.category = _category
  })
  // Extend `/` route
  hook('build:extendRoutes', (routes, resolve) => {
